...
def evaluate_policy(model, env, epoch, calvin_conf_path, eval_log_dir=None, debug=False, create_plan_tsne=False, diverse_inst=False):
    ...
    if diverse_inst:
        with open('/home/yzb/RoboFlamingo/enrich_lang_annotations.json', 'r') as f:
...
def evaluate_policy_ddp(model, env, epoch, calvin_conf_path, eval_log_dir=None, debug=False, create_plan_tsne=False, reset=False, diverse_inst=False):
    ...
    # val_annotations = OmegaConf.load(conf_dir / "annotations/new_playtable_validation.yaml")
    if diverse_inst:
        with open('/home/yzb/RoboFlamingo/lang_annotation_cache.json', 'r') as f:
            val_annotations = json.load(f)
    else:
        val_annotations = OmegaConf.load(conf_dir / "annotations/new_playtable_validation.yaml")

    eval_log_dir = get_log_dir(eval_log_dir)
    with open('/home/yzb/RoboFlamingo/eval_sequences.json', 'r') as f:
...
def generate_zero_shot_instr():
    ...
    for initial_state, eval_sequence in eval_sequences:
        ...
        all_res.append(res)
    with open('/home/yzb/RoboFlamingo/lang_annotation_cache.json', 'w') as f:
...
def save_sequences():
    random.seed(123)
    eval_sequences = get_sequences(NUM_SEQUENCES)
    with open('/home/yzb/RoboFlamingo/eval_sequences.json', 'w') as f:
